(ns firebase.main
  "Kotodama WASM entrypoint for the Firebase clean-room actor (L4) — Clojure port.
  ...
  facts (`firebase.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "firebase")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"firebase.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "firebase-compat" :tier tier :entities entities} 200])
...
(defn start! [] :firebase-compat/ready)
